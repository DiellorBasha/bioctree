...
function updateUIAfterLoad(app)
    ...
    % Update text area
    updateTextArea(app, B);

    % Update the joint axes
    updateJointAxes(app, B, app.UIAxesResponse);
...
% Attach B.Viewer to the grid layout app.GridLayout2
    ...
    % Determine number of rows/cols from definitions
...
%UPDATEJOINTAXES Automatically configure the joint spectral axes
...
%UPDATETEXTAREA  Populate the text area with mesh + time resolution info.
...
% Uses B.Joint if available, otherwise creates Lambda×Omega joint
    ...
    if ~isempty(B.Joint)
        ...
        % Lambda and Omega are automatically created as duals
...
function KernelSliderMoved(app, event)
    updateKernelPreview(app);
end
...
function createDefaultFilter(app)
    ...
    % Gabor is separable: Gaussian in k × Gaussian in ω
    try
...
function updateKernelPreview(app)
    ...
    % This properly uses the kernel from bct.filters.kernels.gabor
    ...
    % Visualizes joint filter on UIAxes_3 (or any axes 'ax')
    ...
    % Create default Joint filter (Lambda × Omega)
    createDefaultFilter(app);

    % Attach mesh viewer (your custom function)
   % attachViewer(app, app.CurrentBCT);

    % Single high-level update
    updateUIAfterLoad(app);
    ...
    % Now that actual eigenvalues exist → update slider ranges
    updateKernelSliders(app, B);
    ...
    % Visualize in Joint UIAxes
    visualizeJointFilter(app, freq_grid, LambdaGrid, F);
        end

function [U, lambda] = solveGeneralized(K, M, numModes, options)
%SOLVEGENERALIZED Solve generalized eigenproblem K*u = λ*M*u
%
% Syntax:
%   [U, lambda] = bct.eigenpairs.solveGeneralized(K, M, k)
%   [U, lambda] = bct.eigenpairs.solveGeneralized(K, M, k, 'EigsOpts', opts)
%
% Inputs:
...
%
% See also: eigs, bct.eigenpairs.normalize
...
try
    ...
    error('bct:eigenpairs:EigsFailed', ...
        'Eigendecomposition failed: %s', ME.message);
end

if flag ~= 0
    warning('bct:eigenpairs:PartialConvergence', ...

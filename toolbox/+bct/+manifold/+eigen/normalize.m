...
%
% Syntax:
%   U = bct.manifold.eigen.normalize(U, M)
%
...
%   - Identical implementation to bct.eigenpairs.normalize
%
...
if p == 0
    ...
    smax = max(s);
    if smax <= 0
        warning('bct:manifold:eigen:InvalidGram', ...
...
if Ierr > 1e-8 * max(1,k)
    warning('bct:manifold:eigen:OrthonormalityDrift', ...

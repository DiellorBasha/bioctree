...
% values: [numModes×1] eigenvalues
s.datasets('values') = struct(...
    'name', 'values', ...
    'path', 'eigen/values', ...
    'dims', 'numModes', ...
    'shape', [NaN, 1], ...
...
% vectors: [numVertices×numModes] eigenvectors
s.datasets('vectors') = struct(...
    'name', 'vectors', ...
    'path', 'eigen/vectors', ...
...
if isfield(eigen, 'values') && isfield(eigen, 'vectors')
    k = size(eigen.vectors, 2);
    if length(eigen.values) ~= k
        warning('bct:manifold:eigen:schema:DimensionMismatch', ...
            'values length (%d) does not match vectors columns (%d)', ...
            length(eigen.values), k);
        tf = false;
    end
end
...
if isfield(eigen, 'attributes') && isfield(eigen.attributes, 'numModes')
    if isfield(eigen, 'values')
        if eigen.attributes.numModes ~= length(eigen.values)
            warning('bct:manifold:eigen:schema:AttributeMismatch', ...
                'attributes.numModes (%d) does not match actual values length (%d)', ...
                eigen.attributes.numModes, length(eigen.values));
            tf = false;
        end
    end
end
...
if isfield(eigen, 'attributes') && isfield(eigen.attributes, 'numVertices')
    if isfield(eigen, 'vectors')
        if eigen.attributes.numVertices ~= size(eigen.vectors, 1)
            warning('bct:manifold:eigen:schema:AttributeMismatch', ...
                'attributes.numVertices (%d) does not match vectors rows (%d)', ...
                eigen.attributes.numVertices, size(eigen.vectors, 1));
            tf = false;
        end
    end
end

...
%
% Syntax:
%   [U, lam] = bct.Manifold.meshFourier(mesh)
%   [U, lam] = bct.Manifold.meshFourier(mesh, k)
%   [U, lam] = bct.Manifold.meshFourier(mesh, k, opts)
%   [U, lam, K, M] = bct.Manifold.meshFourier(...)
%   [U, lam, K, M, D, Ls] = bct.Manifold.meshFourier(...)
%   [U, lam, ...] = bct.Manifold.meshFourier(V, F, ...)  % Alternative syntax
%
% Inputs:
%   mesh - MATLAB surfaceMesh object with Vertices and Faces properties
%          OR bct.Manifold object
%          OR
...
%   [U, lam] = bct.Manifold.meshFourier(mesh, 500);
%   
%   % Using bct.Manifold object
%   manifold = bct.Manifold(struct('V', V, 'F', F));
%   [U, lam] = bct.Manifold.meshFourier(manifold, 500);
%   
%   % Using vertices and faces directly
%   [U, lam] = bct.Manifold.meshFourier(V, F, 500);
%   
%   % Get all outputs including normalized Laplacian
%   [U, lam, K, M, D, Ls] = bct.Manifold.meshFourier(mesh);
%   
...
% Parse inputs - handle (mesh, k, opts), (V, F, k, opts), and bct.Manifold
if nargin < 1
    error('meshFourier:InvalidInput', 'At least one input argument is required');
end

% Determine input type and extract geometry
...
elseif isa(mesh, 'bct.Manifold')
    % Extract from bct.Manifold object
    V = mesh.Vertices;
    F = mesh.Faces;
    N = size(V, 1);
    arg_offset = 0;  % varargin starts with k
elseif isnumeric(mesh) && nargin >= 2 && isnumeric(varargin{1})
    ...
    error('meshFourier:InvalidInput', ...
        'First input must be a surfaceMesh, bct.Manifold, or vertex matrix V (with F as second input)');
end

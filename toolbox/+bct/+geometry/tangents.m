...
%   [N, e1, e2] = bct.geometry.tangents(V, F)
%   [N, e1, e2] = bct.geometry.tangents(___, 'Domain', 'face')    % default
%   [N, e1, e2] = bct.geometry.tangents(___, 'Domain', 'vertex')
%
% Inputs:
...
%   M = bct.Manifold(V, F);
%   [N, e1, e2] = bct.geometry.tangents(M);
%   [N, e1, e2] = bct.geometry.tangents(M, 'Domain', 'face');
%
%   % Vertex tangents from V, F directly
%   [N, e1, e2] = bct.geometry.tangents(V, F, 'Domain', 'vertex');
%
...
p = inputParser;
p.FunctionName = 'bct.geometry.tangents';

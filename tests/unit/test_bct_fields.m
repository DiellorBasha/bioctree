...
ne = bct.field.sizeOfSupport("edge", M);
...
ndf = bct.field.sizeOfSupport("dualFace", M);
assert(ndf == M.numVertices(), 'dualFace count mismatch');

ndv = bct.field.sizeOfSupport("dualVertex", M);
...
assert(~bct.field.isTimeVarying(F), 'should not be time-varying');
...
assert(bct.field.isTimeVarying(F), 'should be time-varying');
...
Fsnap = bct.field.selectTime(F, 25);
assert(~bct.field.isTimeVarying(Fsnap), 'should not be time-varying');
...
% Extract subset
Fsub = bct.field.selectTime(F, 1:2:50);
assert(bct.field.isTimeVarying(Fsub), 'should be time-varying');
...
Fout = bct.field.withTime(F, newTime);
...
F2 = bct.field.withMeta(F1, meta2);
...
Fdouble = bct.field.cast(Fsingle, 'double');
...
nv = M.numVertices();
F = bct.field.infer('vertex', rand(nv, 1));
...
nf = M.numFaces();
F = bct.field.infer('face', rand(nf, 3));
...
nv = M.numVertices();
F = bct.field.infer('vertex', rand(nv, 50));
assert(strcmp(F.valueType, 'scalar'), 'valueType should be scalar');
assert(bct.field.isTimeVarying(F), 'should be time-varying');
...
nv = M.numVertices();
frames = rand(nv, 2, 3);
F = bct.field.infer('vertex', rand(nv, 2), 'frame', frames);
...
nv = M.numVertices();
F = bct.field.infer('vertex', complex(rand(nv, 1), rand(nv, 1)));
...
nv = M.numVertices();
try
    F = bct.field.make('support', 'invalid', ...
                       'valueType', 'scalar', ...
                       'value', rand(nv, 1));
...
fprintf('====================================\n');
fprintf('\nbct.field package implementation complete.\n');

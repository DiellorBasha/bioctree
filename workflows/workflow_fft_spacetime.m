...
plot_spacetime_spectrum(Fk, ax, info, ...
    ...
    TopHeightFrac = 0.2);
%% % 2) Plot with “scatterhistogram-style” layout
plot_spacetime_spectrum_panels(Fk, ax, info, ...
    Units="frequency", ...           % or "angular" / "physical" / "normalized"
    TimePosOnly=true, ...            % show f >= 0
    SpacePosOnly=false, ...          % keep ± spatial for directionality
    TemporalAgg="power-mean", ...    % top marginal
    SpatialAgg="power-mean", ...     % right marginal
    Title="Frequency view: cycles/m vs Hz");
%% 

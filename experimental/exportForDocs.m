...
if options.ExportNormals || options.ExportTangents
    ...
    if options.ExportNormals
        data.geometry.vertex.normals = vg.normals.value;
    end
    
    if options.ExportTangents
        data.geometry.vertex.tangent1 = vg.tangent1.value;
        data.geometry.vertex.tangent2 = vg.tangent2.value;
    end
end
...
if options.ExportNormals || options.ExportTangents
    ...
    data.geometry.face.areas = fg.areas.value;
    data.geometry.face.centroids = fg.centroids.value;
    
    if options.ExportNormals
        data.geometry.face.normals = fg.normals.value;
    end
    
    if options.ExportTangents
        data.geometry.face.tangent1 = fg.tangent1.value;
        data.geometry.face.tangent2 = fg.tangent2.value;
    end
end
...
if options.ExportTopology
    ...
    if isfield(topo, 'isBoundary')
        boundaryHalfedges = find(topo.isBoundary.value);
        if ~isempty(boundaryHalfedges)
            data.topology.hasBoundary = true;
            data.topology.boundaryVertices = unique(topo.tailVertex.value(boundaryHalfedges));
        else
